-- ============================================================
-- NOTIFLY Production Schema
-- PostgreSQL 14+ | Multi-tenant | Idempotency | Outbox | Retry
-- ============================================================
...
-- password_hash is nullable  — Google-only users don't have one.
-- first_name / last_name are nullable — populated from Google profile on OAuth.
-- auth_provider tracks the original sign-up method.
-- google_id is Google's stable "sub" claim from the ID token.
CREATE TABLE IF NOT EXISTS admin_users (
    ...
    google_id     VARCHAR(255) UNIQUE,              -- Google stable user ID (sub)
...
-- ── Transactional Outbox (zero message loss guarantee) ────────────────────────
...
-- ── Notification Logs (per-channel delivery tracking) ─────────────────────────
...
-- ── Event Channel Policy (fallback order) ────────────────────────────────────
...
CREATE INDEX idx_admin_google_id    ON admin_users(google_id) WHERE google_id IS NOT NULL;

-- Notification Requests
CREATE INDEX idx_request_tenant          ON notification_requests(tenant_id);
CREATE INDEX idx_request_tenant_reqid    ON notification_requests(tenant_id, request_id);
...
-- Outbox (critical for polling performance)
